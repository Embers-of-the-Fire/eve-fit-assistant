...
async def generate(data: GeneratorDatasource, collection_cache):
    ...
    for type_pb in collection_cache.types:
        parse_from_icon(type_pb.icon)
    for category_pb in collection_cache.categories:
        parse_from_icon(category_pb.icon)
    for group_pb in collection_cache.groups:
        parse_from_icon(group_pb.icon)
    for market_group_pb in collection_cache.market_groups:
        parse_from_icon(market_group_pb.icon)
    for meta_group_pb in collection_cache.meta_groups:
        parse_from_icon(meta_group_pb.icon)
    for dogma_attribute_pb in collection_cache.dogma_attributes:
