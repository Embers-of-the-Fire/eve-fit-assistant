...
class TestSessionModel:
    def test_channel_validation(self) -> None:
        with pytest.raises(ValidationError, match="Invalid channel"):
            Session(channel="invalid", author="me", description="test")

    def test_testing_channel_is_valid(self) -> None:
        s = Session(channel="testing", author="me", description="test")
        assert s.channel == "testing"

    def test_stable_channel_is_valid(self) -> None:
        s = Session(channel="stable", author="me", description="test")
    ...
    def test_empty_author_raises(self) -> None:
        with pytest.raises(ValidationError, match="author must not be empty"):
            Session(channel="testing", author="", description="test")

    def test_whitespace_author_raises(self) -> None:
        with pytest.raises(ValidationError, match="author must not be empty"):
            Session(channel="testing", author="   ", description="test")

    def test_empty_description_raises(self) -> None:
        with pytest.raises(ValidationError, match="description must not be empty"):
            Session(channel="testing", author="me", description="")
...
class TestSessionStore:
    def test_init_creates_session_file(self, tmp_path: pytest.fixture) -> None:
        ...
        store.init(channel="testing", author="pipeline", description="test session")
        assert store.session_path.is_file()

    def test_init_overwrite_without_force_raises(self, tmp_path: pytest.fixture) -> None:
        store = SessionStore(tmp_path)
        store.init(channel="testing", author="pipeline", description="first session")
        with pytest.raises(SessionExistsError, match="A session already exists"):
            store.init(channel="testing", author="pipeline", description="second session")
    ...
    def test_load_roundtrip(self, tmp_path: pytest.fixture) -> None:
        store = SessionStore(tmp_path)
        original = store.init(channel="testing", author="pipeline", description="desc")
        loaded = store.load()
    ...
    def test_discard_removes_file(self, tmp_path: pytest.fixture) -> None:
        store = SessionStore(tmp_path)
        store.init(channel="testing", author="pipeline", description="test")
        assert store.session_path.is_file()
        store.discard()
    ...
    def test_staged_initializes_empty(self, tmp_path: pytest.fixture) -> None:
        store = SessionStore(tmp_path)
        session = store.init(channel="testing", author="pipeline", description="test")
    ...
    def test_init_populates_all_fields(self, tmp_path: pytest.fixture) -> None:
        store = SessionStore(tmp_path)
        session = store.init(channel="stable", author="admin", description="Release session")
        assert session.channel == "stable"
        assert session.author == "admin"
        assert session.description == "Release session"
        assert session.committed is False
