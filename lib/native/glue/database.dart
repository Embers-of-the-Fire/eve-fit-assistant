...
class NativeDatabase {
  ...
  static Future<NativeDatabase> init() async {
    ...
    final dogmaAttr = await File(
      '${storage.path}/dogmaAttributes.pb2',
    ).readAsBytes();
    final dogmaEffect = await File(
      '${storage.path}/dogmaEffects.pb2',
    ).readAsBytes();
    ...
    final buffs = await File(
      '${storage.path}/dbuffcollections.pb2',
    ).readAsBytes();
  ...
  native.CalculateOutput calculate(schema.Fit fit) =>
      native.calculate(db: _raw, fit: fit);

  Map<int, double> getTypeAttr(int typeID) =>
      native.getTypeAttr(db: _raw, typeId: typeID);
}
