...
schema.Fit intoNativeFit({
  ...
  skills: skills,
  dynamicItems: fit.dynamicItems.map(
    (key, value) => MapEntry(
      key,
      schema.DynamicItem(
...
schema.Module _intoNativeModules(local.Fit fit) => schema.Module(
  ...
  tacticalMode: fit.tacticalModeID.map(
    (id) => _intoNativeItem(
      item: local.SlotItem(
        itemID: id,
        chargeID: null,
        state: local.SlotState.active,
      ),
      index: 0,
    ),
  ),
...
List<schema.Implant> _intoNativeImplants(List<local.SlotItem?> items) =>
    items.enumerate().filterMap((e) {
      if (e.$2 == null || e.$2!.state == local.SlotState.passive) return null;
      return schema.Implant(itemId: e.$2!.itemID, index: e.$1);
    }).toList();

List<schema.Booster> _intoNativeBoosters(List<local.SlotItem?> items) =>
    items.enumerate().filterMap((e) {
      if (e.$2 == null || e.$2!.state == local.SlotState.passive) return null;
      return schema.Booster(itemId: e.$2!.itemID, index: e.$1);
    }).toList();
