...
extension UnitExt on UnitType {
  ...
  String format(double value) => switch (this) {
    ...
    UnitType.inverseAbsolutePercent || UnitType.inversedModifierPercent =>
      '${((1 - value) * 100).toStringAsFixed(0)} %',
    UnitType.modifierPercent =>
      '${value >= 1 ? '+' : ''}${((value - 1) * 100).toStringAsFixed(0)} %',
    UnitType.oreUnits ||
    UnitType.fittingSlots ||
    UnitType.slot => value.toStringAsFixed(0),
    UnitType.groupId =>
      GlobalStorage().static.groups[value.toInt()]?.nameZH ??
          value.toStringAsFixed(0),
    UnitType.typeId =>
      GlobalStorage().static.types[value.toInt()]?.nameZH ??
          value.toStringAsFixed(0),
    ...
    UnitType.datetime => formatDate(fromDaysSinceEpoch(value.toInt()), [
      yyyy,
      '-',
      mm,
      '-',
  ...
  double displayNum(double value) => switch (this) {
    ...
    UnitType.inversedModifierPercent => (1 - value) * 100,
