...
/// Completion of the most recent localization database close.
///
...
Future<void> deleteWebLocalizationDatabases() async {
  await _lastClose;
  await OpfsBlobStore.instance.deleteTree(kLocalizationDbOpfsRoot);
}
...
Future<SqliteDatabase?> openWebLocalizationDb(ResourceBlobProxy proxy) async {
  ...
  final dbName = localizationDbNameForHash(entry.contentHash);
  final opfs = OpfsBlobStore.instance;
  final dbFilePath = localizationDbFilePath(dbName);
  ...
  try {
    ...
    debug(
      "OPFS is unavailable despite cross-origin isolation ($e); localized names are unavailable.",
    );
    return null;
  }
  ...
  if (existingSize != null && expectedSize > 0 && existingSize != expectedSize) {
    warning(
      "Cached OPFS localization database has unexpected size"
      " ($existingSize != $expectedSize); re-copying.",
    );
    await opfs.deleteTree("$kLocalizationDbOpfsRoot/$dbName");
    existingSize = null;
  }
  ...
  unawaited(_lastClose.then((_) => _pruneStaleDatabases(opfs, dbName)));
...
/// pre-staged OPFS copy is unused and the opened database is empty, so
/// localized names will be unavailable.
final class _LoggingWebSqliteOpenFactory extends WebSqliteOpenFactory {
  _LoggingWebSqliteOpenFactory({required super.path, required super.sqliteOptions});
  ...
  Future<ConnectToRecommendedResult> connectToWorker(WebSqlite sqlite, String name) async {
    ...
    if (result.implementation.storage == StorageMode.opfs) {
      debug("Localization database storage: ${result.implementation.name}.");
    } else {
      warning(
        "Localization database opened without OPFS (${result.implementation.name});"
...
Future<void> _pruneStaleDatabases(OpfsBlobStore opfs, String keepName) async {
  try {
    final stale = <String>{};
    for (final path in await opfs.list(kLocalizationDbOpfsRoot)) {
      final segments = path.split("/");
      if (segments.length < 2) continue;
      final dir = segments[1];
      if (isStaleLocalizationDbDir(dir, keepName: keepName)) {
        stale.add("$kLocalizationDbOpfsRoot/$dir");
      }
    }
