...
class BundleServicePaths {
  ...
  Future<IList<BundleValidationError>> validate() async {
    Future<BundleValidationError?> validateForSingle(String filePath, bool shouldBeFile) async {
      final realPath = p.join(bundlePath, filePath);
      final isFile = await File(realPath).exists();
      final isDir = await Directory(realPath).exists();
      if (!isFile && !isDir) {
    ...
    final validated = await Future.wait(
      ...
      ].map((v) => validateForSingle(v.$1, v.$2)),
    );
  ...
  String getNativePath() {
    return p.join(bundlePath, _staticPath, _staticNativePath);
  }
