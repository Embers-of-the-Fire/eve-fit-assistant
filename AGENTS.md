...
- `_release.yml` also builds the Linux variants in a dedicated `linux` job (`.#linux` dev shell). The job is `continue-on-error: true`: it is exercised by the `D-CI-App Release` / `D-Full CI` / `V-Test` flows but never blocks the APK build, remote publish, tagging, or the surrounding check run. The AppImage and the native zip archive are GitHub-Releases-only: they are attached as release assets by the `tag` job when the artifact exists and CI merges only the android fragment into the release registry, although the registry tooling supports a `linux` platform namespace (`LinuxArtifacts` in `release_index.proto`, `x build linux` fragment, remote publish blob dispatch).
