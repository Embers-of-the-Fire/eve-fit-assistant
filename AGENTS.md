...
- `site/` is a SvelteKit app deployed to Cloudflare Workers (pnpm workspace). `biome.json` governs JS/TS formatting/linting for this area. `site/platform/` is the exception: an Astro 7 + Svelte-islands SSR app (discussion platform, Cloudflare Worker `efa-platform` with static assets at `platform.efa-tech.dev`, deployed via `wrangler deploy --config dist/server/wrangler.json`) using `@astrojs/cloudflare` v14 (`import { env } from "cloudflare:workers"`, `Astro.locals.cfContext`, global `caches`), Tailwind v4, a D1 binding (`FIT_DB` → `efa-platform`, shared with `worker/efa-platform-fit-storage`), Astro server endpoints under `src/pages/api/`, and full-HTML edge caching of `/post/<request_id>` via the Cache API in `src/middleware.ts`.
