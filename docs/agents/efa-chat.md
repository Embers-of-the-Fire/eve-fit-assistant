...
| `config.rs` | `ChatProviderKind` (`OpenAiCompatible`/`Anthropic`/`DeepSeek`, each with a default base URL and a `prompt_dir()` name: `openai`/`anthropic`/`deepseek`); `PromptLanguage` (`En`/`Zh`, `from_locale` maps any non-`zh*` tag to `En`); `ChatProviderConfig` (provider / api_key / base_url / model / language / system_prompt / max_turns / proxy) with eager validation. A blank `base_url` resolves to the provider default (`resolved_base_url`). `proxy` is a `ProxyRouting` (`Default` / `Direct` / `Proxy(ProxyConfig)`) resolved by the Dart host from the desktop system-proxy settings (`with_proxy`, blank ignored; `with_direct_routing` for host-bypassed endpoints); `ProxyConfig` carries the per-scheme proxy URLs (`http`/`https`/`all`) plus the `no_proxy`/GNOME-ignore-hosts bypass list. `http_client()`/`build_http_client()` build a reqwest client for it (`Default` yields `None` so the caller keeps rig's default client, which already reads the standard proxy env vars; `Direct` yields a client with `ClientBuilder::no_proxy()` so a bypassed endpoint never falls back to the proxy env vars; `Proxy` yields a per-URL `Proxy::custom` matcher layered on `no_proxy()` — reqwest follows redirects inside the client, so every request URL, redirect targets included, is routed on its own: a redirect to a bypassed host goes direct and a cross-scheme redirect picks up that scheme's proxy; the bypass matcher mirrors the GNOME `ignore-hosts` semantics of `system_proxy.dart`; ignored on wasm, where the browser applies its own proxy). Prompts are bundled from `prompt/` at compile time (`include_str!`); `prompt_bundle()` yields the per-(provider, language) constraint/appendix fragments — `prompt/constraint/system/{lang}.prompt` (shared rules), `prompt/constraint/provider/<dir>/{lang}.prompt` and `prompt/appendix/provider/<dir>/{lang}.prompt` (per-provider, e.g. the DeepSeek DSML strict-ASCII rule plus its examples appendix). `system_prompt` holds *extra sections* appended last (`with_system_prompt`, blank ignored); `full_system_prompt(context)` delegates assembly to `prompt::render_system_prompt`; `max_turns` is the multi-turn depth (tool-call roundtrips per turn, `DEFAULT_MAX_TURNS` = 20, `with_max_turns` overrides, 0 ignored). Tool descriptions are bundled the same way under `prompt/tool/<tool-name>/{en,zh}.prompt` (one directory per rig tool, named by the tool's `NAME`) and selected by the session language via `PromptLanguage::pick`. |
...
| `models.rs` | `list_models(provider, api_key, base_url, routing)` — thin wrapper over rig's native `ModelListingClient` per provider (auth headers and pagination handled by rig; `routing` is the `ProxyRouting` fed to `build_http_client` like the session config); results sorted and deduped by id. Note: OpenAI-compatible listing goes through `openai::Client` (the Responses-ext client), because rig's `OpenAIModelLister` is bound to that client type. |
...
- `ChatConfig.provider` (`ChatProvider` enum) selects the rig provider; blank `base_url`
  means the provider default. `ChatConfig.proxy` is a `ChatProxyRouting`
  (`SystemDefault`/`Direct`/`Proxy{httpUrl, httpsUrl, allUrl, bypass}`) resolved by Dart
  (`chatProxyRoutingFor` in
  `lib/features/chat/provider.dart`, backed by
  `lib/features/remote_content/system_proxy*.dart`: env vars first, GNOME gsettings
  fallback on Linux). `Direct` preserves the host's bypass decision — a proxy-bypassed
  endpoint must not fall back to reqwest's env-var proxy handling, so the crate builds
  the client with `ClientBuilder::no_proxy()`. `Proxy` carries the full per-scheme proxy
  URLs and bypass list rather than the single proxy resolved for the endpoint: reqwest
  follows redirects inside the client, so the crate's per-URL `Proxy::custom` matcher
  re-routes every request (redirect to a bypassed host → direct; cross-scheme redirect →
  that scheme's proxy). `list_available_models` takes the same
  value. `create`, `set_model`,
