...
| `src/config.rs` | `ChatProviderKind` (`OpenAiCompatible`/`Anthropic`/`DeepSeek`, each with a default base URL); `ChatProviderConfig` (provider / api_key / base_url / model / system_prompt / max_turns) with eager validation. A blank `base_url` resolves to the provider default (`resolved_base_url`); the base system prompt is bundled from `prompt/system.prompt.txt` (`BASE_SYSTEM_PROMPT`, persona + manual-tool usage, `include_str!`); `system_prompt` holds *extra sections* appended after the base (`with_system_prompt`, blank ignored), and each provider may declare a `system_prompt_extra()` (`prompt/<provider>.prompt.txt`, e.g. the DeepSeek DSML-ASCII note) appended last; `full_system_prompt()` composes base + extra sections + provider extra; `max_turns` is the multi-turn depth (tool-call roundtrips per turn, `DEFAULT_MAX_TURNS` = 20, `with_max_turns` overrides, 0 ignored). |
...
- The system prompt is baked in per-turn via `Agent::preamble`, not stored in history. It is
  composed as: bundled base (`prompt/system.prompt.txt`) + Dart-supplied extra sections +
  provider extra. The Dart side passes only the dynamic in-app link manifest through
