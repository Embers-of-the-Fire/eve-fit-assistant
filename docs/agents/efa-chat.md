...
- `ChatConfig.provider` (`ChatProvider` enum) selects the rig provider; blank `base_url`
  ...
  (`SystemDefault`/`Direct`/`Proxy{url}`) resolved by Dart (`chatProxyRoutingFor` in
  ...
  the client with `ClientBuilder::no_proxy()`. `list_available_models` takes the same
