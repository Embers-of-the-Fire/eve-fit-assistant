...
<svelte:head>
    <title>{t("report.bug.title")} — {t("brand.name")}</title>
</svelte:head>

<section class="py-24 sm:py-32 relative overflow-hidden">
    <div class="absolute top-0 right-0 w-80 h-80 bg-eve-red/5 rounded-full blur-3xl"></div>
    ...
    <div class="mx-auto max-w-2xl px-6 relative">
        ...
        <div class="mb-10">
            <div class="inline-flex items-center gap-2 rounded-full border border-eve-red/20 bg-eve-red/5 px-4 py-1.5 text-xs text-eve-red tracking-widest uppercase">
                <span class="h-1.5 w-1.5 rounded-full bg-eve-red animate-[twinkle_2s_ease-in-out_infinite]"></span>
                {t("report.bug.title")}
            </div>
            <p class="mt-2 text-sm text-eve-text-muted">{t("report.bug.description")}</p>
        </div>
        ...
        {:else}
            <form onsubmit={handleSubmit} class="space-y-6">
                ...
                <div class="rounded-lg border border-eve-border bg-eve-surface p-6 sm:p-8 space-y-6">
                    ...
                    <label class="block">
                        <span class="block text-xs font-medium text-eve-text-muted mb-1.5">{t("report.form.bug.summary")}</span>
                        <textarea
                            bind:value={summary}
                            rows="2"
                        ...
                        ></textarea>
                        {#if hasFieldError("summary")}
                            <span class="mt-1 block text-xs text-eve-red">{getFieldError("summary")}</span>
                        {/if}
                    </label>

                    <label class="block">
                        <span class="block text-xs font-medium text-eve-text-muted mb-1.5">{t("report.form.bug.steps")}</span>
                        <textarea
                            bind:value={steps}
                            rows="3"
                            class="w-full rounded border border-eve-border bg-eve-bg px-3.5 py-2.5 text-sm text-eve-text placeholder:text-eve-text-muted/50 focus:border-eve-gold focus:outline-none transition-colors resize-y"
                        ></textarea>
                        {#if hasFieldError("steps")}
                            <span class="mt-1 block text-xs text-eve-red">{getFieldError("steps")}</span>
                        {/if}
                    </label>

                    <label class="block">
                        <span class="block text-xs font-medium text-eve-text-muted mb-1.5">{t("report.form.bug.expected")}</span>
                        <textarea
                            bind:value={expected}
                            rows="2"
                            class="w-full rounded border border-eve-border bg-eve-bg px-3.5 py-2.5 text-sm text-eve-text placeholder:text-eve-text-muted/50 focus:border-eve-gold focus:outline-none transition-colors resize-y"
                        ></textarea>
                        {#if hasFieldError("expected")}
                            <span class="mt-1 block text-xs text-eve-red">{getFieldError("expected")}</span>
                        {/if}
                    </label>

                    <label class="block">
                        <span class="block text-xs font-medium text-eve-text-muted mb-1.5">{t("report.form.bug.actual")}</span>
                        <textarea
                            bind:value={actual}
                            rows="2"
                            class="w-full rounded border border-eve-border bg-eve-bg px-3.5 py-2.5 text-sm text-eve-text placeholder:text-eve-text-muted/50 focus:border-eve-gold focus:outline-none transition-colors resize-y"
                        ></textarea>
                        {#if hasFieldError("actual")}
                            <span class="mt-1 block text-xs text-eve-red">{getFieldError("actual")}</span>
                        {/if}
                    </label>
                    ...
                    <label class="block">
                        <span class="block text-xs font-medium text-eve-text-muted mb-1.5">{t("report.form.bug.platform")}</span>
                        <select
                            bind:value={platform}
                            class="w-full rounded border border-eve-border bg-eve-bg px-3.5 py-2.5 text-sm text-eve-text focus:border-eve-gold focus:outline-none transition-colors"
                        >
                            {#each platforms as p}
