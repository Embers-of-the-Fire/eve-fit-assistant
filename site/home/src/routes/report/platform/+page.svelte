...
<svelte:head>
    <title>{t("report.platform.title")} — {t("brand.name")}</title>
</svelte:head>

<section class="py-24 sm:py-32 relative overflow-hidden">
    <div class="absolute top-0 right-0 w-80 h-80 bg-eve-cyan/5 rounded-full blur-3xl"></div>
    ...
    <div class="mx-auto max-w-2xl px-6 relative">
        ...
        <div class="mb-10">
            <div class="inline-flex items-center gap-2 rounded-full border border-eve-cyan/20 bg-eve-cyan/5 px-4 py-1.5 text-xs text-eve-cyan tracking-widest uppercase">
                <span class="h-1.5 w-1.5 rounded-full bg-eve-cyan animate-[twinkle_2s_ease-in-out_infinite]"></span>
                {t("report.platform.title")}
            </div>
            <p class="mt-2 text-sm text-eve-text-muted">{t("report.platform.description")}</p>
        </div>
        ...
        {:else}
            <form onsubmit={handleSubmit} class="space-y-6">
                ...
                <div class="rounded-lg border border-eve-border bg-eve-surface p-6 sm:p-8 space-y-6">
                    <label class="block">
                        <div class="flex items-stretch rounded border {hasFieldError('title') ? 'border-eve-red' : 'border-eve-border'} bg-eve-bg focus-within:border-eve-gold transition-colors">
                            <span class="flex items-center px-3.5 py-3 text-base font-semibold text-eve-gold whitespace-nowrap select-none border-r border-eve-border">
                                {t("report.form.platform_feedback.prefix")}
                            </span>
                            <input
                                type="text"
                                bind:value={title}
                    ...
                    <label class="block">
                        <span class="block text-xs font-medium text-eve-text-muted mb-1.5">{t("report.form.platform_feedback.body")}</span>
                        <textarea
                            bind:value={body}
                            rows="6"
                            placeholder={t("report.form.platform_feedback.body.placeholder")}
